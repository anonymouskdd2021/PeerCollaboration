...
parser.add_argument('--cooperation_type', default='cooperation_embed_entropy', type=str)
...
def cooperation_embed_entropy(model, epoch):
    while True:
        try:
...
def cooperation_embed_L1norm(model, epoch):
    while True:
        try:
            checkpoint = torch.load('%s/ckpt%d_%d.t7' % (args.savedir, args.i - 1, epoch))['net']
            break
    ...
    for epoch in range(args.epochs):
        ...
        while(True):
            try:
                ...
                time.sleep(5)
        del state
        elif args.cooperation_type == 'embed_entropy':
            cooperation_embed_entropy(model, epoch)
        elif args.cooperation_type == 'embed_L1norm':
            cooperation_embed_L1norm(model, epoch)
        else:
            print('error', '*'*50)
