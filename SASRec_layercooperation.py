...
parser.add_argument('--datapath', type=str, default='../Data/Session/ml20m_removecold5_seq.csv',
                    help='data path')
...
parser.add_argument('--cooperation_type', default='alllayer_entropy', type=str)
...
def sigmoid(x):
    return args.b/(1+np.exp(-1 * args.a * x))


def cooperation_onlymiddle(model, epoch):
    while True:
        try:
    ...
    for i, (key, u) in enumerate(model.state_dict().items()):
        if 'embed' in key or 'final' in key:
            ...
            continue
        elif ('fc' in key or 'cnn' in key) and 'weight' in key:
            w = sigmoid(entropy(u) - entropy(checkpoint[key]))
        elif 'SelfAttention.layer_norm' in key and 'weight' in key:
            w = sigmoid(entropy(u) - entropy(checkpoint[key]))
        model_cooperation[key] = u * w + checkpoint[key] * (1 - w)
    model.load_state_dict(model_cooperation)


def cooperationing_onlyembed(model, epoch):
    while True:
        try:
            checkpoint = torch.load('%s/ckpt%d_%d.t7' % (args.savedir, args.i - 1, epoch))['net']
            break
...
def cooperation_onlyfinal(model, epoch):
    while True:
        try:
            checkpoint = torch.load('%s/ckpt%d_%d.t7' % (args.savedir, args.i - 1, epoch))['net']
            break
    ...
    model_cooperation = collections.OrderedDict()
    for i, (key, u) in enumerate(model.state_dict().items()):
        if 'final' in key and 'weight' in key:
            w = sigmoid(entropy(u) - entropy(checkpoint[key]))
            model_cooperation[key] = u * w + checkpoint[key] * (1 - w)
        elif 'final' in key and 'bias' in key:
            model_cooperation[key] = u * w + checkpoint[key] * (1 - w)
        else:
            model_cooperation[key] = u
    ...
    model.load_state_dict(model_cooperation)
    del checkpoint
    del model_cooperation


def cooperation_alllayer_entropy(model, epoch):
    while True:
        try:
            checkpoint = torch.load('%s/ckpt%d_%d.t7' % (args.savedir, args.i - 1, epoch))['net']
            time.sleep(20)
    ...
    for i, (key, u) in enumerate(model.state_dict().items()):
        if 'embed' in key:
            w = sigmoid(entropy(u) - entropy(checkpoint[key]))
        elif ('fc' in key or 'final' in key or 'cnn' in key) and 'weight' in key:
            w = sigmoid(entropy(u) - entropy(checkpoint[key]))
        elif 'SelfAttention.layer_norm' in key and 'weight' in key:
            w = sigmoid(entropy(u) - entropy(checkpoint[key])) # normal
        model_cooperation[key] = u * w + checkpoint[key] * (1 - w)
    while(True):
        try:
            model.load_state_dict(model_cooperation)
...
def cooperation_alllayer_L1norm(model, epoch):
    while True:
        try:
            checkpoint = torch.load('%s/ckpt%d_%d.t7' % (args.savedir, args.i - 1, epoch))['net']
            break
    ...
    print('*'*25, ' start: ', epoch, ' ', '*'*25)
    for i, (key, u) in enumerate(model.state_dict().items()):
        L1_sum = u.detach().abs().sum() + checkpoint[key].detach().abs().sum()
        if ('embed' in key) or (('fc' in key or 'final' in key or 'cnn' in key) and 'weight' in key) or\
                ('layer_norm' in key and 'weight' in key):
            w = sigmoid((u.detach().abs().sum() / L1_sum - checkpoint[key].detach().abs().sum() / L1_sum).data.cpu().numpy())
        model_cooperation[key] = u * float(w) + checkpoint[key] * (1 - float(w))
    while(True):
        try:
            model.load_state_dict(model_cooperation)
            break
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        #     'acc(hit@1)': acc
        # }
        # torch.save(state, '%s/best_%s_%s.t7' % (args.savedir, args.i, model_para['dilations']))
    ...
    for epoch in range(args.epochs):
        ...
        while(True):
            try:
                ...
                time.sleep(5)
        if args.cooperation_type == 'onlymiddle':
            cooperation_onlymiddle(model, epoch)
        elif args.cooperation_type == 'onlyembed':
            cooperationing_onlyembed(model, epoch)
        elif args.cooperation_type == 'onlyfinal':
            cooperation_onlyfinal(model, epoch)
        elif args.cooperation_type == 'alllayer_entropy':
            cooperation_alllayer_entropy(model, epoch)
        elif args.cooperation_type == 'alllayer_L1norm':
            cooperation_alllayer_L1norm(model, epoch)
        else:
            print('error', '*'*50)
